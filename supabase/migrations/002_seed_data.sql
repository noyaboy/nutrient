...
INSERT INTO plan_items (title, description, frequency, category, sort_order, target_count, is_active) VALUES
  ...
  ('19:00 晚餐 + 低 FODMAP 蔬菜', '蛋白質 40-45g + 菠菜、櫛瓜等低 FODMAP 蔬菜（十字花科留給午餐）。順序：纖維→蛋白→碳水。脂肪：橄欖油 1 大匙（14g）入菜或涼拌 + 堅果一把 30g（~15g 脂肪）≈ 20-25g。隨餐服用維他命 C 500mg（1 錠，午餐膠原蛋白已含 ~160mg Vit C，每日總計 ~660mg 已達 RDA，無需 1000mg 避免過量）。固體食物睡前 3-4hr 結束', 'daily', '飲食', 11, 1, true),
  ...
  ('15:30 下午點心', '豌豆蛋白 ~20g 粉（≈16g 蛋白）。非乳製植物蛋白，中速消化。下午點心時段服用，分散蛋白質攝取壓力。避免 19:00 晚餐 45-50g + 20:00 再補 16g = 1.5hr 內超過 60g 造成消化不良', 'daily', '飲食', 9, 1, true),
  ('21:30-22:00 睡前補充品', '甘胺酸 3g + 蘇糖酸鎂 + 甘胺酸鎂 100mg + Ashwagandha 600mg（嚴格 8 週用 / 4 週停）。⚠️ 第 6 週起密切觀察情緒變化：若出現情緒冷漠（Anhedonia）或早晨無力起床 → 立即進入停用期。提前至 21:30-22:00 服用，為腎臟保留排尿緩衝時間，避免半夜起床中斷睡眠', 'daily', '補充品', 14, 1, true),
  ...
  -- === All-day items ===
  ('全天 蛋白質 146g+ 目標', '訓練前乳清 27g + 午餐 40-45g（含每週 1-2 次草飼牛肉 200-300g 替代雞胸）+ 下午豌豆 16g + 晚餐 40-45g ≈ 130-140g。每餐達亮氨酸門檻 2.5-3g。每日 4-5 餐均勻分配，總計 146g+（2.0g/kg）。牛肉日額外提供血基質鐵、B12、天然肌酸', 'daily', '飲食', 19, 1, true),
  ...
  ('全天 鈣攝取確認（目標 1000mg）', '每日確認鈣攝取是否達標 1000mg。主要來源：希臘優格 200-300g（~200-300mg）+ 牛奶/起司 + 深綠蔬菜 + 豆腐。⚠️ 若當日飲食鈣攝取確認不足 → 下午 14:00-15:00 隨輕食補 1 錠鈣片 500mg（Nature Made Ca+D3+K2）。禁止睡前服用（鈣與鎂競爭 DMT1 載體，同服降低兩者吸收率）。與 15:00 銅間隔 1hr+', 'daily', '飲食', 23, 1, true),
