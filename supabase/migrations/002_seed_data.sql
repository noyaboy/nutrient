...
INSERT INTO plan_items (title, description, frequency, category, sort_order, target_count, is_active) VALUES
  ...
  ('10:00 運動', '一上半身A/二下半身A/四上半身B/五下半身B。三 VO2 Max 間歇。六/日 Zone 2 有氧 45-60 分鐘。每次 60-90 分鐘。🚫 冷水浴嚴格限制：重訓日（一二四五）及 VO2 Max 日（三）絕對禁止冷水浴（抑制 mTOR/IGF-1 肌肥大信號 + 線粒體適應信號）。僅週六/日 Zone 2 日可在早晨執行冷水浴（與運動間隔 4hr+）', 'daily', '運動', 5, 1, true),
  ...
  -- === Midday (12:00-15:00) ===
  ('12:00 午餐 + 訓練後補充品', '蛋白質 45-50g（正餐食物）+ 肌酸 5g（CGN Creatine Monohydrate）+ 十字花科蔬菜（切碎靜置 40 分鐘最大化蘿蔔硫素）+ 碳水 80-105g（訓練日，建議使用冷卻再加熱米飯以保留抗性澱粉 RS3）。脂肪：橄欖油 1 大匙（14g）+ 酪梨半顆（~15g）≈ 30g。魚油 3 顆、D3 2000IU（5+2）、K2 MK-7 100mcg、葉黃素 20mg、膠原蛋白肽 10-15g、CoQ10 100-200mg。每日脂肪目標 80-90g（22-25% 總熱量）', 'daily', '飲食', 7, 1, true),
  ...
  ('15:00-16:00 銅 2mg 補充', '銅 2mg（Solaray Bisglycinate）嚴格單獨空腹服用，不與任何礦物質補劑同服。空腹可最大化吸收率，避免與鋅、鈣、鐵等礦物質競爭。與晚餐鋅間隔 4hr+（鋅僅週二、六服用）。若空腹不適可搭配少量水果（非含鈣/鐵食物）', 'daily', '補充品', 10, 1, true),
  ...
  -- === Evening (19:00+) ===
  ('19:00 晚餐 + 低 FODMAP 蔬菜', '蛋白質 40-45g + 菠菜、櫛瓜等低 FODMAP 蔬菜（十字花科留給午餐）。順序：纖維→蛋白→碳水。脂肪：橄欖油 1 大匙（14g）入菜或涼拌 + 堅果一把 30g（~15g 脂肪）≈ 20-25g。隨餐服用維他命 C 500mg（1 錠，午餐膠原蛋白已含 ~160mg Vit C，每日總計 ~660mg 已達 RDA，無需 1000mg 避免過量）。固體食物睡前 3-4hr 結束', 'daily', '飲食', 13, 1, true),
  ...
  -- === All-day items ===
  ('全天 蛋白質 146g+ 目標', '訓練前乳清 27g + 午餐 40-45g（含每週 1-2 次草飼牛肉 200-300g 替代雞胸）+ 下午豌豆 16g + 晚餐 40-45g ≈ 130-140g。每餐達亮氨酸門檻 2.5-3g。每日 4-5 餐均勻分配，總計 146g+（2.0g/kg）。牛肉日額外提供血基質鐵、B12、天然肌酸', 'daily', '飲食', 21, 1, true),
  ...
  ('全天 鈣攝取確認（目標 1000mg）', '每日確認鈣攝取是否達標 1000mg。主要來源：希臘優格 200-300g（~200-300mg）+ 牛奶/起司 + 深綠蔬菜 + 豆腐。⚠️ 若當日飲食鈣攝取確認不足 → 下午 14:00-15:00 隨輕食補 1 錠鈣片 500mg（Nature Made Ca+D3+K2）。禁止睡前服用（鈣與鎂競爭 DMT1 載體，同服降低兩者吸收率）。與 15:00 銅間隔 1hr+', 'daily', '飲食', 25, 1, true),
...
INSERT INTO products (name, description, usage, price, url, store, category, brand, origin, specs, nutrition, purchase_note, sort_order) VALUES (
  ...
  '線上可訂（常溫配送）。每日 1 錠（500mg），500 錠可用 16 個月。⚠️ 維他命 C 開瓶後易氧化：務必旋緊瓶蓋、存放陰涼乾燥處、避免陽光直射。開瓶超過 6 個月後效力可能下降，建議在瓶身標記開瓶日期。若擔心氧化可考慮分裝至小密封罐。午餐膠原蛋白已提供 ~160mg Vit C，晚餐 1 錠 500mg 即可。',
  3
);
