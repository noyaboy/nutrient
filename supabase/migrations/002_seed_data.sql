...
INSERT INTO plan_items (title, description, frequency, category, sort_order, target_count, is_active) VALUES
  ...
  ('11:15 咖啡 + L-Theanine', '起床後 90-135 分鐘再喝（09:00 起床 → 10:30-11:15 飲用，避免干擾皮質醇覺醒反應，且與 09:15 B群間隔 2hr+）。咖啡因 200-300mg（約 1-2 杯黑咖啡）+ L-Theanine 200mg（NOW Foods Double Strength 200mg，iHerb，1:1 比例 A 級 nootropic 組合，消除焦慮、增強專注）。15:00 後禁止咖啡因（半衰期 5-6 小時影響睡眠）。⚠️ B群吸收保護：咖啡因利尿作用加速水溶性 B 群（尤其 B1）排出，與 09:15 B群服用間隔至少 2 小時。⚠️ 綠茶日規定：凡飲用綠茶之日，一律停用 L-Theanine 補劑（200mg 素食膠囊無法精準減半，綠茶 2-3 杯已提供天然 L-Theanine 40-90mg）', 'daily', '飲食', 6, 1, true),
  ...
  ('14:00-15:00 銅 2mg 補充', '銅 2mg（Solaray Bisglycinate）隨低鈣/低鐵小點心服用（如少量水果、幾片餅乾），避免空腹服用引發噁心嘔吐。不與鋅、鈣、鐵等礦物質補劑同服。⚠️ 時間安排：12:00 午餐（鈣隨餐）→ 14:00-15:00 銅（間隔 2-3hr）→ 19:00 晚餐最後一口鋅（間隔 4-5hr）。⚠️ 牛肉日銅補劑：牛肉本身含銅極低（150g ≈ 0.1-0.2mg），若當日堅果攝取充足（≥30g，含銅 ~0.3-0.5mg）可免補銅，否則仍需補充。遵從性優先：不再堅持「嚴格空腹」，以不引起嘔吐感為最高原則', 'daily', '補充品', 10, 1, true),
  ('15:30 下午點心', 'Tryall 豌豆蛋白 ~20g 粉（≈16g 蛋白）。非乳製植物蛋白，中速消化。下午點心時段服用，分散蛋白質攝取壓力。與 Tryall 乳清同品牌，品質一致。⚠️ 牛肉日加 1 顆水煮蛋：牛肉日晚餐取消雞蛋後，1 顆蛋（~6.3g 蛋白）移至此處與豌豆蛋白同食（合計 22.3g，仍在合理範圍）', 'daily', '飲食', 11, 1, true),
...
INSERT INTO plan_items (title, description, frequency, category, sort_order, target_count, is_active) VALUES
  ...
  ('【每半年】健康檢測', '每半年抽血：(1) 腎功能：BUN + Creatinine + eGFR + Cystatin C。🔴 絕對前提：抽血前 7 天停用肌酸 + 48-72hr 暫停高強度重訓（否則 eGFR 偽低估）。eGFR <90 分級處置：首次 <90 → 下修至 1.6g/kg，停肌酸，3 個月複檢；連續兩次 <90 → 強制 1.2g/kg，轉介腎臟科。(2) 肝功能：ALT + AST。Ashwagandha 新品牌第 4/12 週加測。DILI 不限品牌/批次：疲累加重、食慾不振、皮膚微黃、右上腹不適（肝臟位置） → 即刻停用抽血。(3) 甲狀腺：TSH + Free T4（Ashwagandha 可能提升 T4）。(4) 其他：維他命 D（目標 40-60 ng/mL）、鋅銅比（10-15:1）、睪固酮、SHBG、皮質醇、空腹血糖、HbA1c、胰島素。⚠️ ALT/AST 異常停用順序：首位 Ashwagandha → 其次 CoQ10/葉黃素，停用 2 週複檢。🚫 Ashwagandha 禁忌：SSRIs/SNRIs 血清素藥物禁用 + 甲亢/甲狀腺藥物禁用 + 自體免疫疾病（橋本氏甲狀腺炎、類風濕性關節炎、紅斑性狼瘡等）完全禁用', 'weekly', '一般', 6, 0, true),
