...
INSERT INTO plan_items (title, description, frequency, category, sort_order, target_count, is_active) VALUES
  ...
  ('09:15 訓練前營養', '香蕉/地瓜 + 乳清蛋白 ~30g 粉（≈27g 蛋白）+ B群 1 顆（水溶性，早晨隨餐提供全天能量代謝）。⚠️ 地瓜建議前晚用電子鍋蒸好冷藏（產生抗性澱粉 RS3），隔日微波 30 秒加溫即可食用，抗性澱粉仍保留大部分', 'daily', '飲食', 3, 1, true),
  ...
  ('22:00 睡前補充品', '甘胺酸 3g + 蘇糖酸鎂 + 甘胺酸鎂 100mg + Ashwagandha 600mg（嚴格 8 週用 / 4 週停，在瓶身標記開始日與第 56 天停用日）。🚫 血清素藥物禁忌：若正在服用抗憂鬱劑（SSRIs/SNRIs）或任何影響血清素的藥物 → 必須立即停用 Ashwagandha，可能誘發血清素綜合徵（Serotonin Syndrome）。⚠️ 第 6 週起密切觀察情緒變化：若出現情緒冷漠（Anhedonia）或早晨無力起床 → 立即進入停用期。停用期替代：甘胺酸鎂 + Cyclic Sighing 維持睡眠品質。22:00 服用確保與 19:00 晚餐蛋白質間隔 3hr+（甘胺酸與蛋白質競爭吸收），同時為腎臟保留排尿緩衝時間', 'daily', '補充品', 15, 1, true),
  ('Ashwagandha 週期管理（8 週用 / 4 週停）', '在瓶身標記「開始日」與「第 56 天停用日」。📋 週期檢查：第 1-5 週正常服用 / 第 6 週起每日自評情緒冷漠（Anhedonia）、早晨無力起床 → 任一症狀立即停用 / 第 8 週（56 天）準時進入停用期 / 停用 4 週（28 天）替代方案：甘胺酸鎂 + Cyclic Sighing。🚫 禁忌：服用 SSRIs/SNRIs 或血清素藥物者禁用。⚠️ ALT/AST 異常 → 首位停用本品。每半年健檢時確認肝功能指標', 'daily', '補充品', 16, 1, true),
  ...
  ('全天 鈣攝取確認（目標 1000mg）', '每日確認鈣攝取是否達標 1000mg。主要來源：希臘優格 200-300g（~200-300mg）+ 深綠蔬菜 + 豆腐。⚠️ 若當日飲食鈣攝取確認不足 → 午餐後 1hr（13:00）補 1 錠鈣片 500mg（Nature Made Ca+D3+K2），與午餐脂溶性維生素（魚油/D3/K2/葉黃素）間隔 1hr+ 避免競爭吸收。⚠️ 補鈣當日：午餐 D3 改為 500IU（1/4 錠）而非完全停用，鈣片已含 D3 150IU → 當日合計 650IU，維持穩定血清 25(OH)D 濃度。禁止睡前服用（鈣與鎂競爭 DMT1 載體）。13:00 鈣 → 16:00 銅 → 19:00 鋅，各間隔 3hr+', 'daily', '飲食', 25, 1, true),
...
INSERT INTO products (name, description, usage, price, url, store, category, brand, origin, specs, nutrition, purchase_note, sort_order) VALUES (
  ...
  '線上可訂（常溫配送）。純鈣備用品（鈣從食物攝取為主），250 錠可用非常久。⚠️ 補鈣當日注意：本品每錠含 D3 150IU，午餐獨立 D3 改為 500IU（1/4 錠，非完全停用），當日合計 650IU 維持穩定血清 25(OH)D 濃度。碳酸鈣+檸檬酸鈣錠劑常溫陰涼處即可穩定保存。適合放辦公桌或隨身攜帶，確保 13:00 補鈣時段可取用。',
  2
);
...
INSERT INTO products (name, description, usage, price, url, store, category, brand, image_url, rating, review_count, sku, specs, nutrition, purchase_note, sort_order) VALUES (
  ...
  'iHerb 必買品項（SKU: NOW-00420）。每日 1 顆隨 09:15 早餐（訓練前營養餐）服用，100 顆可用 3 個多月（每季補貨 1 次）。購買連結：https://tw.iherb.com/pr/now-foods-b-50-100-veg-capsules/39670。水溶性維生素，隨餐服用提升吸收率。尿液變黃為正常現象（B2 代謝）',
  32
);
...
INSERT INTO products (name, description, usage, price, url, store, category, brand, image_url, rating, review_count, sku, specs, nutrition, purchase_note, sort_order) VALUES (
  ...
  'iHerb 直送。⚠️ 8 週用期（56 天）每日 1 顆，每瓶 90 顆可完成 1 個完整週期（56 顆用 + 剩 34 顆留下一輪）。建議在瓶身標記「開始日」與「第 56 天停用日」。停用期 4 週（28 天）替代方案：甘胺酸鎂 + Cyclic Sighing 呼吸法維持睡眠品質。⚠️ 第 6 週起留意情緒冷漠 → 立即停用。若 ALT/AST 異常 → 首位停用本品。🚫 血清素藥物禁忌：若正在服用抗憂鬱劑（SSRIs/SNRIs）或任何影響血清素的藥物 → 禁用本品（血清素綜合徵風險）',
  36
);
