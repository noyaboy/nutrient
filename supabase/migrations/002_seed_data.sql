...
INSERT INTO plan_items (title, description, frequency, category, sort_order, target_count, is_active) VALUES
  ...
  ('09:15 訓練前營養', '地瓜（推薦）或香蕉 + 乳清蛋白 ~30g 粉（≈27g 蛋白）+ B群 1 顆（活化型態 Coenzyme B-Complex：甲鈷胺 B12 + 5-MTHF 葉酸 + P5P B6，水溶性需隨餐服用確保吸收）。⚠️ 地瓜建議前晚用電子鍋蒸好冷藏（產生抗性澱粉 RS3），隔日微波 30 秒加溫即可食用，抗性澱粉仍保留大部分。⚠️ B群勿空腹：需搭配地瓜或香蕉，食物基質減緩胃排空提升吸收', 'daily', '飲食', 3, 1, true),
  ...
  ('16:00-17:00 銅 2mg 補充', '銅 2mg（Solaray Bisglycinate）隨低鈣/低鐵小點心服用（如少量水果、幾片餅乾），避免空腹服用引發噁心嘔吐。不與鋅、鈣、鐵等礦物質補劑同服。⚠️ 時間安排：14:00 鈣 → 16:00 銅（間隔 2hr）→ 19:00 鋅（間隔 3hr）。遵從性優先：不再堅持「嚴格空腹」，以不引起嘔吐感為最高原則', 'daily', '補充品', 10, 1, true),
  ...
  -- === All-day items ===
  ('全天 蛋白質 113-123g+（1.5-1.7g/kg）', '訓練前乳清 27g + 午餐 35-40g（含每週 1-2 次草飼牛肉 150-180g 替代雞胸）+ 下午豌豆 16g + 晚餐 35-40g ≈ 113-123g。每餐達亮氨酸門檻 2.5-3g，單餐 ≤40g 避免 BUN 飆升與腸道產氣。每日 4-5 餐均勻分配，總計約 1.5-1.7g/kg（健康腎功能範圍）。⚠️ 牛肉日嚴格零蛋：牛肉 150-180g ≈ 30-36g 蛋白，晚餐不加蛋（36g + 1蛋 6.3g = 42.3g 超過 40g 上限）。牛肉日額外提供血基質鐵、B12、天然肌酸。⚠️ eGFR <90 警示：若腎絲球過濾率 <90（排除肌酸偽陽性後）→ 立即下修總蛋白至 1.6g/kg（≈ 117g/day），每餐 ≤35g，停止肌酸補充，密切監測 BUN/Creatinine。⚠️ 肌酸偽陽性：抽血前 3-5 天停用肌酸，避免 Creatinine 偽高導致 eGFR 低估', 'daily', '飲食', 21, 1, true),
...
INSERT INTO plan_items (title, description, frequency, category, sort_order, target_count, is_active) VALUES
  ...
  ('鋅 15mg 每日補充', '每日隨晚餐服用鋅 15mg（Picolinate 錠劑 1 錠）。補充品 15mg + 飲食鋅 ~10-15mg = 每日總計 25-30mg（安全低於 UL 40mg/日）。與 16:00 銅間隔 3hr+。⚠️ 牛肉日豁免：草飼牛肉日（每週 1-2 次）當晚取消鋅補劑，牛肉 150-180g 已含鋅 6-9mg，無需額外補充。⚠️ 鋅鈣競爭：晚餐避免高鈣食物（希臘優格移至午餐/點心），高鈣 >200mg 明顯抑制鋅吸收', 'daily', '補充品', 27, 1, true);
...
INSERT INTO products (name, description, usage, price, url, store, category, brand, image_url, rating, review_count, sku, specs, nutrition, purchase_note, sort_order) VALUES (
  ...
  'iHerb 直送。每日 5g，454g 可用 90 天（3 個月）。⚠️ 建議設定每 3 個月定期補貨提醒，避免斷供影響肌力維持。⚠️ 抽血前 3-5 天停用：肌酸補充會使 Creatinine（肌酐）偽陽性升高 → eGFR 被低估，停用 3-5 天可恢復真實基線值，避免誤判腎功能異常。CGN 自有品牌常有額外折扣。搭配其他 iHerb 品項湊免運。',
  22
);
...
INSERT INTO products (name, description, usage, price, url, store, category, brand, image_url, rating, review_count, sku, specs, nutrition, purchase_note, sort_order) VALUES (
  ...
  'iHerb 直送。✅ 15mg 錠劑（非 50mg 膠囊），可精確控量。每日 1 錠，120 錠可用 4 個月。含飲食鋅（蛋 ~5mg + 肉 ~5-12mg）每日總鋅攝取 25-32mg，安全低於 UL 40mg。⚠️ 牛肉日豁免：牛肉 150-180g 已含鋅 6-9mg，當晚不服鋅補劑。與 16:00 銅間隔 3hr+（晚餐 19:00 服用）',
  25
);
...
INSERT INTO products (name, description, usage, price, url, store, category, brand, image_url, rating, review_count, sku, specs, nutrition, purchase_note, sort_order) VALUES (
  ...
  '每日服用 1 顆（1000 IU）。目標血清 25(OH)D 40-60 ng/mL。血檢達標+每日晨光曝曬→可進一步減量。✅ 1000IU 軟凝膠，每日 1 顆免切，避免原 2000IU 軟凝膠「無法掰半」的劑量風險',
  ...
  'iHerb 直送。✅ 每日 1 顆（1000IU），無需切割。180 顆可用 6 個月。軟凝膠不可掰半（內容物為油脂液態），故選擇 1000IU 規格避免劑量翻倍風險。脂溶性，務必隨含油脂午餐服用。',
  27
);
