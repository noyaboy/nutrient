...
INSERT INTO plan_items (title, description, frequency, category, sort_order, target_count, is_active) VALUES
  ...
  ('16:00-17:00 銅 2mg 補充', '銅 2mg（Solaray Bisglycinate）隨低鈣/低鐵小點心服用（如少量水果、幾片餅乾），避免空腹服用引發噁心嘔吐。不與鋅、鈣、鐵等礦物質補劑同服。⚠️ 時間安排：14:00 鈣 → 16:00 銅（間隔 2hr）→ 17:00 膠原蛋白（補鈣日限定）→ 19:00 晚餐最後一口鋅（間隔 3hr+）。遵從性優先：不再堅持「嚴格空腹」，以不引起嘔吐感為最高原則', 'daily', '補充品', 10, 1, true),
...
INSERT INTO products (name, description, usage, price, url, store, category, brand, image_url, rating, review_count, sku, specs, nutrition, purchase_note, sort_order) VALUES (
  ...
  'iHerb 直送。每日 5g，454g 可用 90 天（3 個月）。⚠️ 建議設定每 3 個月定期補貨提醒，避免斷供影響肌力維持。🔴 抽血絕對前提：健檢前 3-5 天必須停用肌酸，否則 Creatinine 偽陽性升高 → eGFR 被低估 → 可能誤判腎功能異常。只有停用後的 eGFR 才能反映真實腎功能。CGN 自有品牌常有額外折扣。搭配其他 iHerb 品項湊免運。',
  22
);
...
INSERT INTO products (name, description, usage, price, url, store, category, brand, image_url, rating, review_count, sku, specs, nutrition, purchase_note, sort_order) VALUES (
  ...
  '水解海洋膠原蛋白肽 + 玻尿酸 + 維他命 C（每 6.5g 含 80mg Vit C）。午餐服用 10-15g 已提供 ~120-160mg Vit C，與晚餐 500mg 合計每日 ~660mg，無需額外疊加。🔴 補鈣日移至 17:00 服用（與 14:00 鈣片在腸道錯開）',
  '每日 10-15g 隨午餐（已含 Vit C ~160mg）。🔴 補鈣日改至 17:00 服用（銅之後、晚餐之前）',
  'NT$1,279 / 206g',
  ...
  'iHerb 直送。每日 10-15g，206g 約可用 2-3 週。已內含維他命 C（~160mg/份），與晚餐 500mg Vit C 合計每日 ~660mg（充足且不過量）。🔴 補鈣日膠原蛋白移至 17:00+：14:00 鈣片 + 膠原蛋白 160mg VitC 同在腸道 → 結石風險，移至銅之後（16:00）、晚餐之前（19:00）獨立服用。搭配其他 iHerb 品項湊免運。',
  30
);
