...
INSERT INTO plan_items (title, description, frequency, category, sort_order, target_count, is_active) VALUES
  ...
  ('晚餐 橄欖油 28g（2 大匙）',
   ...
   'daily', '脂肪', 28, 1, true),
  ('09:05 碘鹽 1g',
   '500ml 室溫水 + 碘鹽 1g（電子秤測量 0.1g 精度）+ 檸檬汁。碘鹽取代海鹽確保碘攝取。⚠️ 全日鈉預算（WHO <2400mg/日）：晨間 1g（~400mg 鈉）+ 午晚餐烹調碘鹽合計 1-1.5g（~400-600mg 鈉）+ 泡菜 30g（~200mg 鈉）+ 茶葉蛋 1 顆（~250mg 鈉）+ 食物天然鈉 ~500mg ≈ 總計 1750-1950mg。⚠️ 午晚餐烹調碘鹽嚴格控制在合計 1-1.5g 內。⚠️ 全日碘鹽僅提供 ~20-33mcg 碘（台灣食鹽法規標準），必須搭配每日紫菜/海苔 3-4 片（另含約 36-172mcg）確保穩定達 RDA 150mcg。⚠️ 碘攝取變異大（每日 56-205mcg），低紫菜日可能不足 RDA。建議每半年驗尿碘確認',
   'daily', '礦物質', 2, 1, true),
  ('藍莓 50g（抗氧化）',
...
INSERT INTO products (name, description, usage, price, url, store, category, brand, origin, specs, nutrition, purchase_note, sort_order) VALUES (
  ...
  '碳酸鈣 + 檸檬酸鈣雙鈣源。每日午餐服用（含板豆腐日）。注意：本品含 D3 150IU/錠，與獨立 D3 2000IU 疊加後總計 2150IU 仍在安全範圍。K2 僅 10mcg/錠（不足日需量，已改用獨立 K2 MK-7 100mcg）',
  '每日隨午餐服用 1 錠（+500mg 鈣）。日常 K2 已改用獨立 K2 MK-7 產品',
  'NT$759 / 250 錠',
