...
INSERT INTO plan_items (title, description, frequency, category, sort_order, target_count, is_active) VALUES
  ...
  ('09:05 補水 & 電解質', '起床後盡快補水。500ml 室溫水 + 碘鹽 3-5g（使用食品電子秤精確測量，提供碘 ~60-165mcg）+ 檸檬汁。可搭配晨光曝曬同時進行。⚠️ 碘攝取策略：RDA 150mcg/日，台灣市售碘鹽含碘 ~20-33mcg/g。每日 09:05 碘鹽 3-5g 僅提供部分碘需求（~60-165mcg），其餘碘來源：烹調用碘鹽（每日總鹽攝取 <6g WHO 建議）+ 每週 2-3 次海帶/紫菜（富含碘 500-3000mcg/g 乾重）。⚠️ 十字花科蔬菜含硫代葡萄糖苷（Goitrogens），會競爭甲狀腺對碘的吸收 → 高十字花科攝取者碘需求更高。⚠️ 週六/日 Zone 2 有氧日：09:05 補水改用電解質粉沖泡 500ml（CGN Sport Hydration），訓練中持續補充電解質水', 'daily', '飲食', 2, 1, true),
  ('09:15 NMN + TMG（空腹）', '香蕉/地瓜 + 乳清蛋白 ~30g 粉（≈27g 蛋白）', 'daily', '補充品', 2, 1, false),
  ('09:15 訓練前營養', '⏰ 09:05 先補水（500ml 室溫水 + 碘鹽 3-5g 用電子秤測量 + 檸檬汁）→ 09:15 訓練前營養。地瓜（推薦）或香蕉 + 乳清蛋白 ~30g 粉（≈27g 蛋白）+ B群 1 顆（活化型態 Coenzyme B-Complex：甲鈷胺 B12 + 5-MTHF 葉酸 + P5P B6，水溶性需隨餐服用確保吸收）。⚠️ 地瓜建議前晚用電子鍋蒸好冷藏（產生抗性澱粉 RS3），隔日微波 30 秒加溫即可食用，抗性澱粉仍保留大部分。⚠️ B群勿空腹：需搭配地瓜或香蕉，食物基質減緩胃排空提升吸收。⚠️ 順序：先喝水再吃固體食物+B群，確保胃中有食物基質時 B群才進入', 'daily', '飲食', 3, 1, true),
...
INSERT INTO plan_items (title, description, frequency, category, sort_order, target_count, is_active) VALUES
  ...
  ('晚餐 橄欖油 28g（2 大匙）',
   ...
   'daily', '脂肪', 28, 1, true),
  ('09:05 碘鹽 3-5g',
   '500ml 室溫水 + 碘鹽 3-5g（使用食品電子秤精確測量，1/4 茶匙 ≈ 1.4g 誤差過大不可靠）+ 檸檬汁。碘鹽取代海鹽以確保碘攝取（甲狀腺功能必需）。⚠️ 碘攝取計算：台灣碘鹽含碘 ~20-33mcg/g，3-5g 碘鹽提供 ~60-165mcg。加上烹調用碘鹽，每日碘攝取可達 RDA 150mcg+。⚠️ 十字花科蔬菜高攝取者碘需求更高（硫代葡萄糖苷干擾甲狀腺碘吸收）。建議每週 2-3 次在午餐加入海帶/紫菜增補碘攝取。⚠️ Zone 2 日（週六/日）09:05 補水改用電解質粉，碘鹽併入其他餐次',
   'daily', '礦物質', 2, 1, true),
  ('藍莓 50g（抗氧化）',
...
INSERT INTO products (name, description, usage, price, url, store, category, brand, origin, specs, nutrition, purchase_note, sort_order) VALUES (
  ...
  '{}'::jsonb,
  '線上可訂（常溫配送）。2kg 約可用 3 個月以上。⚠️ 務必確認現有庫存為「加碘」版本（包裝標示「碘化鉀」成分）。海鹽/玫瑰鹽碘含量極低，十字花科攝取量高時碘不足風險大增。⚠️ 改用食品電子秤（0.1g 精度）測量碘鹽：1/4 茶匙量勺 ≈ 1.4g（非 1g），誤差 40% 不可接受。電子秤已列入設備清單。',
  9
);
...
INSERT INTO products (name, description, usage, price, url, store, category, purchase_note, sort_order) VALUES (
  ...
  'Amazon', 'equipment',
  '一次性購買，Amazon 或蝦皮。精度 0.1g，最大秤量至少 5kg。$300-800 即可。⚠️ 碘鹽每日用量 3-5g 需精確測量（1/4 茶匙量勺誤差 40% 不可靠），電子秤為唯一可靠工具。',
  38
);
...
INSERT INTO products (name, description, usage, price, url, store, category, purchase_note, sort_order) VALUES (
  '1/4 茶匙不鏽鋼量勺',
  '精確測量每日碘鹽。⚠️ 注意：1/4 茶匙 ≈ 1.4g（非 1g），誤差 40%，不適合精確測量碘鹽。碘鹽建議改用食品電子秤（0.1g 精度，已列入設備清單）。量勺可作為其他調味料粗略量測用途',
  '（備用）其他調味料粗略量測',
  'NT$50-150',
  'https://www.amazon.com/measuring-spoons-stainless-steel/s?k=measuring+spoons+stainless+steel',
  'Amazon', 'equipment',
  '（可選）若已有食品電子秤則無需另購量勺。一次性購買，Amazon 或蝦皮或烘焙用品店。⚠️ 碘鹽精確測量請使用食品電子秤（0.1g 精度），量勺誤差過大（1/4 茶匙 ≈ 1.4g 非 1g）。',
  41
);
