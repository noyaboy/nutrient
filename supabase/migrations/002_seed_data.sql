...
-- Matches live DB state as of 2026-03-01 (migrations 003-082 applied incrementally on live DB)
------------------------------------------------------------
...
INSERT INTO plan_items (title, description, frequency, category, sort_order, target_count, is_active) VALUES
  ...
  ('09:15 訓練前營養', '香蕉 1 根 + 乳清蛋白 ~30g 粉（≈27g 蛋白）+ 肌酸 5g（溶於乳清一起沖泡）+ 奇亞籽 15g（~5g 纖維 + 2.5g ALA omega-3）。⚠️ 距訓練僅 45 分鐘，只吃流質乳清+好消化的香蕉+奇亞籽（先泡水 10 分鐘形成凝膠再加入乳清）。🔴 訓練日纖維補強：奇亞籽 15g（5g 纖維）+ 洋車前子殼 8-10g（6.4-8g）= 額外 11-13g 纖維，搭配蔬菜 5-8g 達標 35g+。地瓜需 60-90 分鐘消化，若要吃請提前至 08:00', 'daily', '飲食', 3, 1, true),
  ...
  ('14:00 咖啡 + L-Theanine（午餐後 2hr）', '☕ 午餐後 2 小時飲用（~14:00）。咖啡因 150-200mg + L-Theanine 200mg（1:1 A 級 nootropic 組合，保護睡眠品質）。15:00 後禁止咖啡因。✅ 無論當天是否飲用綠茶，喝咖啡就必須同步服用 L-Theanine。⚠️ 咖啡必須在午餐後 2 小時以上飲用：咖啡多酚（Chlorogenic acid）在胃腸道內螯合非血基質鐵（non-heme iron），抑制吸收率達 60-90%。30 分鐘內飲用仍會損失 60-70% 鐵吸收（胃排空需 90-120 分鐘）。2 小時後飲用時，午餐鐵已大部分被十二指腸吸收，影響降至 10-20%。⚠️ 起床後至少 60-90 分鐘再攝取咖啡因（皮質醇覺醒反應 CAR 結束後）', 'daily', '飲食', 6, 1, true),

  -- === Midday (12:00-15:00) ===
  ('12:00 午餐 + 訓練後補充品', '蛋白質 35-40g（正餐完整蛋白質，不含膠原蛋白）+ 蔬菜（建議使用白米或冷卻再加熱白米飯以減少植酸干擾鐵吸收）。隨餐服用：魚油 2 顆（鮭魚日 1 顆）、D3 2000IU（2 顆）、葉黃素 20mg、膠原蛋白肽 10-15g（額外，含 VitC ~120-160mg）、維他命C 500mg 1 錠（搭配鐵質食物最大化吸收）、B群 1 顆（⚠️ 隔日服用——P5P 50mg/日長期有周邊神經病變風險）。🔴 CoQ10 與 K2 已移至晚餐（28g 橄欖油提供更充足脂質微膠粒）。⚠️ 午餐搭配維他命C食物（彩椒、花椰菜）促進鐵吸收', 'daily', '飲食', 7, 1, true),
  ...
  ('15:00 綠茶 EGCG 2-3 杯', '午餐後 3hr+ 再飲用（~15:00），高脂高蛋白午餐胃排空需 3-4hr，3hr 內 EGCG 仍會螯合鐵鋅。改用低咖啡因綠茶（白毫銀針或老白毫），配合 L-theanine 天然組合促進專注。15:30 前喝完（咖啡因 cutoff 統一 15:30）。綠茶天然 L-Theanine（40-90mg）為額外放鬆紅利', 'daily', '飲食', 8, 1, true),
  ...
  ('15:30 下午點心', 'Tryall 豌豆蛋白 22g 粉（≈16g 蛋白）+ 雞蛋 1 顆（牛肉日從晚餐移入）。💊 16:00 服用鈣片 1 錠（500mg）——低鐵點心時段，不干擾任何礦物質吸收。⚠️ 訓練日改用此組合。休息日可改為燕麥 80g + 藍莓 50g。⚠️ 牛肉日：僅 1 顆蛋（6.3g 蛋白）+ 豌豆 16g = 22.3g', 'daily', '飲食', 11, 1, true),
  ...
  -- === All-day items ===
  ('全天 蛋白質 142-152g+（1.9-2.1g/kg）', '訓練前乳清 27g + 午餐 41-44g + 下午豌豆 16g + 晚餐 38-45g + 睡前酪蛋白 20g ≈ 142-152g。每餐 ≤45g，每日 5 餐均勻分配，總計約 1.9-2.1g/kg。🔴 睡前酪蛋白（22:00）填補 19:00-08:00 長達 13hr 的蛋白質斷層，多項 RCT 證實睡前酪蛋白顯著提升夜間肌肉蛋白合成率', 'daily', '飲食', 21, 1, true),
  ('全天 膳食纖維 35-45g', '洋蔥、大蒜、蘆筍、燕麥、扁豆、酪梨、冷卻米飯/地瓜（抗性澱粉）。搭配發酵食物（優格、泡菜 ≤30g/日）增強腸道多樣性。⚠️ 訓練日注意：低纖維碳水替換（白米/義大利麵/去皮馬鈴薯）可能使纖維降至 20-25g。訓練日補充：奇亞籽 15g（5g 纖維）+ 洋車前子殼 8-10g（6.4-8g 纖維）= 額外 11-13g，搭配蔬菜 5-8g 達標 35g+', 'daily', '飲食', 22, 1, true),
  ...
  ('全天 鐵吸收最佳化確認（目標 RDA 8mg）', '每日確認鐵吸收最佳化（男性 RDA 8mg）。🔴 鐵吸收促進因子：(1) 維他命C 午餐 500mg + 晚餐 250mg 隨餐（促進非血基質鐵吸收 2-6 倍）✅。(2) 動物蛋白「肉類因子」（MFP）促進鐵吸收。(3) 鈣片已移至 16:00 下午點心——午餐/晚餐鐵均不再被鈣抑制 ✅。🔴 鐵吸收抑制因子：(1) 咖啡 15:00（午餐後 3hr），影響 5-10% ✅。(2) 綠茶 15:30（3.5hr 後），影響有限 ✅。(3) 植酸：午餐使用白米。(4) 草酸（菠菜）：僅影響自身鐵。主要鐵來源：牛肉、雞蛋、菠菜、雞肝 2×/週 50g。⚠️ 每半年驗血清鐵蛋白 Ferritin 目標 50-150 ng/mL', 'daily', '飲食', 31, 1, true);
...
INSERT INTO products (name, description, usage, price, url, store, category, brand, origin, specs, nutrition, purchase_note, sort_order) VALUES (
  ...
  '{}'::jsonb,
  '每日必須 ~105g。線上可訂（常溫配送）。1.13kg 堅果約 3.5 週。另購：南瓜籽 500g + 核桃 500g + 葵花籽 500g（全聯/Costco）。🔴 鮭魚日跳過巴西堅果',
  11
);
