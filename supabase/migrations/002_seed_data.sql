...
INSERT INTO plan_items (title, description, frequency, category, sort_order, target_count, is_active) VALUES
  ...
  ('全天 鈣攝取確認（目標 1000mg）', '每日確認鈣攝取是否達標 1000mg。主要來源：希臘優格 200-300g（~200-300mg）+ 深綠蔬菜 + 豆腐。⚠️ 若當日飲食鈣攝取確認不足 → 下午 14:00-15:00 隨輕食補 1 錠鈣片 500mg（Nature Made Ca+D3+K2）。禁止睡前服用（鈣與鎂競爭 DMT1 載體，同服降低兩者吸收率）。與 15:00 銅間隔 1hr+', 'daily', '飲食', 25, 1, true),
