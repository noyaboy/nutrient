...
INSERT INTO plan_items (title, description, frequency, category, sort_order, target_count) VALUES
  ...
  ('09:00 訓練後早餐 + 補充品', '乳清蛋白 ~40g 粉（≈36g 蛋白）+ 肌酸 5g + 碳水 80-105g；魚油 3 顆、D3 2000IU（5+2）、D3+K2（不額外補鈣片，鈣從食物攝取）、維他命 C、葉黃素 20mg', 'daily', '飲食', 5, 1),
  ('13:00 午餐', '蛋白質 40-50g + 十字花科蔬菜（脹氣時換菠菜/櫛瓜等低FODMAP，蛋白質降至30g）', 'daily', '飲食', 7, 1),
  ('14:00 銅 2mg', '與鋅間隔 4hr+（鋅在 18:00 晚餐服用）。5+2：週一至五服用，週末休息', 'daily', '補充品', 8, 1),
  ('18:00 晚餐 + 鋅 25mg', '蛋白質 + 十字花科蔬菜（脹氣換低FODMAP）+ 原型碳水。進食順序：纖維→蛋白→碳水。鋅 25mg 隨餐（5+2）。社交聚餐可彈性延後', 'daily', '飲食', 9, 1),
  ('18:30 餐後散步 15 分鐘', '控血糖關鍵', 'daily', '運動', 10, 1),
  ('20:00 藍光管理', '調暗燈光或佩戴防藍光眼鏡（琥珀色鏡片）', 'daily', '睡眠', 11, 1),
  ('21:00 睡前 希臘優格 + 甘胺酸鎂', '希臘優格 300g（≈28g 蛋白）+ 甘胺酸鎂 200mg。若隔日異常昏沉或夜間腸胃過快→暫停甘胺酸鎂或優格減半', 'daily', '補充品', 12, 1),
...
INSERT INTO plan_items (title, description, frequency, category, sort_order, target_count) VALUES
  ('Zone 2 有氧 3 次', '週三、週六、週日各 45-60 分鐘。心率 60-70% HRmax。腳踏車/飛輪/划船機', 'weekly', '運動', 1, 3),
