...
INSERT INTO plan_items (title, description, frequency, category, sort_order, target_count, is_active) VALUES
  ...
  ('14:00-15:00 銅 2mg 補充', '銅 2mg（Solaray Bisglycinate）隨低鈣/低鐵小點心服用（如少量水果、幾片餅乾），避免空腹服用引發噁心嘔吐。不與鋅、鈣、鐵等礦物質補劑同服。⚠️ 時間安排：12:00 午餐（鈣隨餐）→ 14:00-15:00 銅（間隔 2-3hr）→ 19:00 晚餐最後一口鋅（間隔 4-5hr）。⚠️ 牛肉日免補銅：牛肉 150-180g + 堅果已提供足夠銅，當日取消銅補劑避免逼近 UL 10mg/日。遵從性優先：不再堅持「嚴格空腹」，以不引起嘔吐感為最高原則', 'daily', '補充品', 10, 1, true),
...
INSERT INTO plan_items (title, description, frequency, category, sort_order, target_count, is_active) VALUES
  ...
  ('【每半年】健康檢測', '每半年抽血：(1) 腎功能：BUN + Creatinine + eGFR + Cystatin C。🔴 絕對前提：抽血前 7 天停用肌酸 + 48-72hr 暫停高強度重訓（否則 eGFR 偽低估）。eGFR <90 分級處置：首次 <90 → 下修至 1.6g/kg，永久停肌酸，3 個月複檢；連續兩次 <90 → 強制 1.2g/kg，轉介腎臟科。(2) 肝功能：ALT + AST。Ashwagandha 新品牌第 4/12 週加測。DILI 不限品牌/批次：疲累加重、食慾不振、皮膚微黃、右上腹不適 → 即刻停用抽血。(3) 甲狀腺：TSH + Free T4（Ashwagandha 可能提升 T4）。(4) 其他：維他命 D（目標 40-60 ng/mL）、鋅銅比（10-15:1）、睪固酮、SHBG、皮質醇、空腹血糖、HbA1c、胰島素。⚠️ ALT/AST 異常停用順序：首位 Ashwagandha → 其次 CoQ10/葉黃素，停用 2 週複檢。🚫 Ashwagandha 禁忌：SSRIs/SNRIs 血清素藥物禁用 + 甲亢/甲狀腺藥物禁用 + 自體免疫疾病（橋本氏甲狀腺炎、類風濕性關節炎、紅斑性狼瘡等）完全禁用', 'weekly', '一般', 6, 0, true),
