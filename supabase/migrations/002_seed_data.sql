...
INSERT INTO plan_items (title, description, frequency, category, sort_order, target_count, is_active) VALUES
  ...
  -- === Evening (19:00+) ===
  ('19:00 晚餐 + 低 FODMAP 蔬菜', '蛋白質 40-45g + 菠菜、櫛瓜等低 FODMAP 蔬菜（十字花科留給午餐）。順序：纖維→蛋白→碳水。脂肪：橄欖油 2 大匙（28g）入菜或涼拌。隨餐服用：維他命 C 500mg（半錠）+ 鋅 25mg（半顆，每兩天一次，與 15:00 銅間隔 4hr+）。⚠️ 牛肉日注意：草飼牛肉 200g ≈ 40-50g 蛋白，應減少其他蛋白來源（減蛋/減量），確保單餐 ≤50g 避免腎臟短時間代謝壓力。固體食物睡前 3-4hr 結束', 'daily', '飲食', 13, 1, true),
  ...
  -- === All-day items ===
  ('全天 蛋白質 146g+ 目標', '訓練前乳清 27g + 午餐 40-45g（含每週 1-2 次草飼牛肉 200-300g 替代雞胸）+ 下午豌豆 16g + 晚餐 40-45g ≈ 130-140g。每餐達亮氨酸門檻 2.5-3g。每日 4-5 餐均勻分配，總計 146g+（2.0g/kg）。⚠️ 牛肉日：牛肉 200g ≈ 40-50g 蛋白，晚餐應減少其他蛋白來源（少加蛋），確保單餐 ≤50g。牛肉日額外提供血基質鐵、B12、天然肌酸', 'daily', '飲食', 21, 1, true),
  ...
  ('全天 鈣攝取確認（目標 1000mg）', '每日確認鈣攝取是否達標 1000mg。主要來源：希臘優格 200-300g（~200-300mg）+ 深綠蔬菜 + 豆腐。⚠️ 若當日飲食鈣攝取確認不足 → 午餐後 13:00 隨輕食補 1 錠鈣片 500mg（Nature Made Ca+D3+K2）。⚠️ 補鈣當日：鈣片已含 D3 150IU，午餐獨立 D3 應停用（避免疊加超量，嚴格遵守血檢達標後減量原則）。禁止睡前服用（鈣與鎂競爭 DMT1 載體）。與 15:00 銅間隔 2hr+', 'daily', '飲食', 25, 1, true),
