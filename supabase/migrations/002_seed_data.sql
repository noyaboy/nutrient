...
INSERT INTO plan_items (title, description, frequency, category, sort_order, target_count, is_active) VALUES
  ...
  ('09:05 補水 & 電解質', '起床後盡快補水。500ml 室溫水 + 碘鹽 1g（使用標準 1/4 茶匙量勺精確測量）+ 檸檬汁。可搭配晨光曝曬同時進行。碘鹽取代海鹽以確保碘攝取。⚠️ 週六/日 Zone 2 有氧日：09:05 補水改用電解質粉沖泡 500ml（CGN Sport Hydration），訓練中持續補充電解質水', 'daily', '飲食', 2, 1, true),
  ('09:15 NMN + TMG（空腹）', '香蕉/地瓜 + 乳清蛋白 ~30g 粉（≈27g 蛋白）', 'daily', '補充品', 2, 1, false),
  ('09:15 訓練前營養', '⏰ 09:05 先補水（500ml 室溫水 + 碘鹽 1g + 檸檬汁）→ 09:15 訓練前營養。地瓜（推薦）或香蕉 + 乳清蛋白 ~30g 粉（≈27g 蛋白）+ B群 1 顆（活化型態 Coenzyme B-Complex：甲鈷胺 B12 + 5-MTHF 葉酸 + P5P B6，水溶性需隨餐服用確保吸收）。⚠️ 地瓜建議前晚用電子鍋蒸好冷藏（產生抗性澱粉 RS3），隔日微波 30 秒加溫即可食用，抗性澱粉仍保留大部分。⚠️ B群勿空腹：需搭配地瓜或香蕉，食物基質減緩胃排空提升吸收。⚠️ 順序：先喝水再吃固體食物+B群，確保胃中有食物基質時 B群才進入', 'daily', '飲食', 3, 1, true),
  ...
  ('16:00-17:00 銅 2mg 補充', '銅 2mg（Solaray Bisglycinate）隨低鈣/低鐵小點心服用（如少量水果、幾片餅乾），避免空腹服用引發噁心嘔吐。不與鋅、鈣、鐵等礦物質補劑同服。⚠️ 時間安排：14:00 鈣 → 16:00 銅（間隔 2hr）→ 18:00 膠原蛋白（補鈣日限定，與鈣片間隔 4hr+）→ 19:00 晚餐最後一口鋅（間隔 3hr+）。⚠️ 牛肉日免補銅：牛肉 150-180g + 堅果已提供足夠銅，當日取消銅補劑避免逼近 UL 10mg/日。遵從性優先：不再堅持「嚴格空腹」，以不引起嘔吐感為最高原則', 'daily', '補充品', 10, 1, true),
  ...
  ('Ashwagandha 週期管理（8 週用 / 4 週停）', '在瓶身標記「開始日」與「第 56 天停用日」，建議設定手機鬧鐘提醒停用日期。📋 週期檢查：第 1-5 週正常服用 450mg/日 / 第 6 週起每日自評情緒冷漠（Anhedonia）、早晨無力起床 → 任一症狀立即停用 / 第 8 週（56 天）準時進入停用期 / 停用 4 週（28 天）替代方案：甘胺酸鎂 + Cyclic Sighing。🚫 禁忌：服用 SSRIs/SNRIs 或血清素藥物者禁用。🚫 甲狀腺警示：Ashwagandha 可能提升 T4 水平，甲亢或正服用甲狀腺藥物者禁用（甲狀腺風暴風險）。⚠️ ALT/AST 異常 → 首位停用本品。⚠️ 肝功能追蹤：開始服用新品牌 Ashwagandha 後第 4 週及第 12 週各追蹤一次 ALT/AST（藥源性肝損傷多發於數週內，勿枯等半年健檢）。🚫 感冒/免疫啟動時立即停用：Ashwagandha 具有免疫調節作用，急性感染徵兆出現時 → 立即暫停，待完全康復後再恢復。每半年健檢時確認肝功能 + 甲狀腺指標（TSH、Free T4）', 'daily', '補充品', 16, 1, true),
...
INSERT INTO plan_items (title, description, frequency, category, sort_order, target_count, is_active) VALUES
  ...
  ('鋅 15mg 每日補充', '每日晚餐「最後一口」吞服鋅 15mg（Picolinate 錠劑 1 錠）。最後一口服用而非隨餐混吃，最大化與 16:00 銅離子的時間距離（銅鋅共用轉運蛋白 DMT1/ZIP）。補充品 15mg + 飲食鋅 ~10-15mg = 每日總計 25-30mg（安全低於 UL 40mg/日）。⚠️ 牛肉日豁免：草飼牛肉日（每週 1-2 次）當晚取消鋅補劑，牛肉 150-180g 已含鋅 6-9mg，無需額外補充。⚠️ 鋅鈣競爭：晚餐避免高鈣食物（希臘優格移至午餐/點心），高鈣 >200mg 明顯抑制鋅吸收', 'daily', '補充品', 27, 1, true);
...
INSERT INTO plan_items (title, description, frequency, category, sort_order, target_count, is_active) VALUES
  ...
  ('晚餐 橄欖油 28g（2 大匙）',
   ...
   'daily', '脂肪', 28, 1, true),
  ('09:05 碘鹽 1g',
   '500ml 室溫水 + 碘鹽 1g（使用標準 1/4 茶匙量勺精確測量）+ 檸檬汁。碘鹽取代海鹽以確保碘攝取（甲狀腺功能必需）。碘 RDA 150mcg/day，碘鹽含碘 ~45mcg/g。⚠️ Zone 2 日（週六/日）09:05 補水改用電解質粉，碘鹽併入其他餐次',
   'daily', '礦物質', 2, 1, true),
  ('藍莓 50g（抗氧化）',
...
INSERT INTO products (name, description, usage, price, url, store, category, brand, origin, specs, nutrition, purchase_note, sort_order) VALUES (
  ...
  '線上可訂（常溫配送）。純鈣備用品（鈣從食物攝取為主），250 錠可用非常久。✅ 補鈣當日注意：本品每錠含 D3 150IU，午餐獨立 D3 1000IU 仍正常服用（總計 1150IU 安全低於 UL 4000IU/日）。碳酸鈣+檸檬酸鈣錠劑常溫陰涼處即可穩定保存。適合放辦公桌或隨身攜帶，確保 13:00 補鈣時段可取用。',
  2
);
...
INSERT INTO products (name, description, usage, price, url, store, category, brand, origin, specs, nutrition, purchase_note, sort_order) VALUES (
  ...
  'iHerb 直送。✅ 每日 1 錠（500mg），免切免切藥器。100 錠可用約 3 個月。午餐膠原蛋白已含 ~160mg Vit C，晚餐 1 錠 500mg，每日總計 ~660mg。⚠️ 不再購買 1000mg 版本：軟凝膠/錠劑切半會導致內容物氧化、劑量不準、污染風險。開瓶後標記日期，6 個月內用完。',
  3
);
...
INSERT INTO products (name, description, usage, price, url, store, category, brand, origin, specs, nutrition, purchase_note, sort_order) VALUES (
  ...
  '{}'::jsonb,
  '線上可訂（常溫配送）。2kg 約可用 6 個月以上。⚠️ 務必確認現有庫存為「加碘」版本（包裝標示「碘化鉀」成分）。海鹽/玫瑰鹽碘含量極低，十字花科攝取量高時碘不足風險大增。⚠️ 建議另購標準 1/4 茶匙量勺（不鏽鋼材質，藥局或烘焙用品店），每日精確測量 1g 碘鹽，避免「少許」的模糊估量。',
  9
);
...
INSERT INTO products (name, description, usage, price, url, store, category, brand, origin, specs, nutrition, purchase_note, sort_order) VALUES (
  ...
  '每週 1-2 次作為紅肉來源，替代 1-2 餐雞胸肉（150-180g/次 ≈ 30-36g 蛋白）。⚠️ 牛肉日嚴格零蛋（36g + 1蛋 6.3g = 42.3g 超過 40g 上限）。⚠️ 牛肉日取消鋅補劑（牛肉 150-180g 已含鋅 6-9mg）。補充血基質鐵（植物鐵吸收率僅 2-20%）與 B12。草飼優於穀飼。瘦部位脂肪 <10g/100g。',
  23
);
...
INSERT INTO products (name, description, usage, price, url, store, category, brand, image_url, rating, review_count, sku, specs, nutrition, purchase_note, sort_order) VALUES (
  ...
  'iHerb 直送。每日 5g，454g 可用 90 天（3 個月）。⚠️ 建議設定每 3 個月定期補貨提醒，避免斷供影響肌力維持。🔴 抽血絕對前提：健檢前 7 天必須停用肌酸（24 歲高強度重訓 + 高蛋白 113-123g/day，Creatinine 代謝需更長時間回基準線），否則 Creatinine 偽陽性升高 → eGFR 被低估 → 可能誤判腎功能異常。只有停用後的 eGFR 才能反映真實腎功能。CGN 自有品牌常有額外折扣。搭配其他 iHerb 品項湊免運。',
  22
);
...
INSERT INTO products (name, description, usage, price, url, store, category, brand, image_url, rating, review_count, sku, specs, nutrition, purchase_note, sort_order) VALUES (
  ...
  'iHerb 直送。✅ 15mg 錠劑（非 50mg 膠囊），可精確控量。每日 1 錠，120 錠可用 4 個月。含飲食鋅（蛋 ~5mg + 肉 ~5-12mg）每日總鋅攝取 25-32mg，安全低於 UL 40mg。⚠️ 晚餐「最後一口」吞服：非隨餐混吃，最大化與 16:00 銅的時間距離（銅鋅共用轉運蛋白）。⚠️ 牛肉日豁免：牛肉 150-180g 已含鋅 6-9mg，當晚不服鋅補劑',
  25
);
...
INSERT INTO products (name, description, usage, price, url, store, category, brand, image_url, rating, review_count, sku, specs, nutrition, purchase_note, sort_order) VALUES (
  ...
  '非紅肉日每日 1 顆，下午 16:00 隨低鈣/低鐵小點心服用（少量水果或餅乾，避免空腹噁心）。⚠️ 牛肉日免補銅',
  'NT$217 / 100 顆',
  ...
  'iHerb 直送。非紅肉日每日 1 顆，100 顆可用 4 個月+（因牛肉日免補）。⚠️ 16:00 隨小點心服用（非嚴格空腹）：銅離子空腹刺激性高，易引發噁心嘔吐。⚠️ 牛肉日免補銅：牛肉 150-180g 含銅 ~0.1-0.2mg + 堅果一把含銅 ~0.3-0.5mg + 飲食其他來源，當日銅攝取已足夠，額外補 2mg 可能逼近 UL 10mg/日。與晚餐鋅間隔 3hr+',
  26
);
...
INSERT INTO products (name, description, usage, price, url, store, category, brand, image_url, rating, review_count, sku, specs, nutrition, purchase_note, sort_order) VALUES (
  ...
  'iHerb 直送。每日 1 顆（200mg），120 顆可用 4 個月，與其他 iHerb 品項同步補貨。1:1 比例（咖啡因 200mg + L-Theanine 200mg）為標準用量。⚠️ 綠茶日微調：若 13:00 飲用綠茶 2-3 杯（天然 L-Theanine 40-90mg），當日補劑可減半（100mg）或跳過，避免過度放鬆影響下午專注力',
  29
);

INSERT INTO products (name, description, usage, price, url, store, category, brand, image_url, rating, review_count, sku, specs, nutrition, purchase_note, sort_order) VALUES (
  '膠原蛋白肽 CollagenUP（CGN 206g）',
  '水解海洋膠原蛋白肽 + 玻尿酸 + 維他命 C（每 6.5g 含 80mg Vit C）。午餐服用 10-15g 已提供 ~120-160mg Vit C，與晚餐 500mg 合計每日 ~660mg，無需額外疊加。🔴 補鈣日移至 18:00 服用（與 14:00 鈣片間隔 4hr+）',
  '每日 10-15g 隨午餐（已含 Vit C ~160mg）。🔴 補鈣日改至 18:00 服用（與鈣片間隔 4hr+，晚餐前獨立服用）',
  'NT$1,279 / 206g',
  ...
  'iHerb 直送。每日 10-15g，206g 約可用 2-3 週。已內含維他命 C（~160mg/份），與晚餐 500mg Vit C 合計每日 ~660mg（充足且不過量）。🔴 補鈣日膠原蛋白移至 18:00+：14:00 鈣片必須與膠原蛋白 160mg VitC 間隔 4hr+（結石黃金三角），移至 18:00 晚餐前獨立服用。搭配其他 iHerb 品項湊免運。',
  30
);
...
INSERT INTO products (name, description, usage, price, url, store, category, brand, image_url, rating, review_count, sku, specs, nutrition, purchase_note, sort_order) VALUES (
  ...
  'iHerb 直送。⚠️ 8 週用期（56 天）每日 1 顆，每瓶 90 顆可完成 1 個完整週期（56 顆用 + 剩 34 顆留下一輪）。建議在瓶身標記「開始日」與「第 56 天停用日」，並設定手機鬧鐘提醒。停用期 4 週替代：甘胺酸鎂 + Cyclic Sighing。⚠️ 第 6 週起留意情緒冷漠 → 立即停用。若 ALT/AST 異常 → 首位停用本品。⚠️ 肝功能追蹤：新品牌開始後第 4 週、第 12 週各追蹤 ALT/AST（藥源性肝損傷多發於數週內）。🚫 血清素藥物禁忌（SSRIs/SNRIs）→ 禁用。🚫 甲狀腺警示：甲亢或正服用甲狀腺藥物者禁用（可能提升 T4，甲狀腺風暴風險）。🚫 感冒/免疫啟動時立即暫停，待康復後再恢復',
  36
);
...
INSERT INTO products (name, description, usage, price, url, store, category, brand, image_url, rating, review_count, sku, specs, nutrition, purchase_note, sort_order) VALUES (
  ...
  'iHerb 直送。有氧日每次 1 包，每週 2 次（週六/日），20 包約 10 週。每年需約 5-6 盒（52 週 ÷ 10 週/盒 ≈ 5.2 盒）。建議每 2.5 個月補貨 1 盒，或趁 iHerb 促銷時一次購入 3 盒（半年份）。CGN 自有品牌有折扣。',
  37
);
