...
INSERT INTO plan_items (title, description, frequency, category, sort_order, target_count, is_active) VALUES
  ...
  ('全天 膳食纖維 35-45g', '洋蔥、大蒜、蘆筍、燕麥、扁豆、酪梨、冷卻米飯/地瓜。供腸道菌產生丁酸，維護腸腦軸。每日至少 1 份發酵食物：希臘優格 200-300g（午餐或 15:30 點心，⚠️ 嚴禁晚餐食用：優格鈣 200-300mg 與 19:00 鋅 15mg 競爭吸收載體，必須間隔 2hr+。15:30 點心 → 19:00 鋅 = 3.5hr 間隔 ✓）+ 泡菜 50-100g（晚餐隨餐）。雙菌源增強腸道多樣性', 'daily', '飲食', 22, 1, true),
...
INSERT INTO plan_items (title, description, frequency, category, sort_order, target_count, is_active) VALUES
  ...
  ('鋅 15mg 每日補充', '每日晚餐「最後一口」吞服鋅 15mg（Picolinate 錠劑 1 錠）。最後一口服用而非隨餐混吃，最大化與 16:00 銅離子的時間距離（銅鋅共用轉運蛋白 DMT1/ZIP）。補充品 15mg + 飲食鋅 ~10-15mg = 每日總計 25-30mg（安全低於 UL 40mg/日）。⚠️ 牛肉日豁免：草飼牛肉日（每週 1-2 次）當晚取消鋅補劑，牛肉 150-180g 已含鋅 6-9mg，無需額外補充。⚠️ 鋅鈣競爭：晚餐避免高鈣食物（希臘優格移至午餐/點心），高鈣 >200mg 明顯抑制鋅吸收。⚠️ 優格間隔 2hr+：優格最晚 17:00 前食用完畢（17:00 → 19:00 鋅 = 2hr 間隔），或安排在午餐/15:30 點心更佳', 'daily', '補充品', 27, 1, true);
