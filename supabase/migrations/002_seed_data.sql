...
INSERT INTO plan_items (title, description, frequency, category, sort_order, target_count, is_active) VALUES
  ...
  -- === Midday (12:00-15:00) ===
  ('12:00 午餐 + 訓練後補充品', '蛋白質 45-50g（正餐食物）+ 肌酸 5g（CGN Creatine Monohydrate）+ 十字花科蔬菜（切碎靜置 40 分鐘最大化蘿蔔硫素）+ 碳水 80-105g（訓練日，建議使用冷卻再加熱米飯以保留抗性澱粉 RS3）。脂肪：橄欖油 1 大匙（14g）+ 酪梨半顆（~15g）≈ 30g。魚油 3 顆、D3 1000IU（每日半顆）、K2 MK-7 100mcg、葉黃素 20mg、膠原蛋白肽 10-15g、CoQ10 200mg。每日脂肪目標 80-90g（午餐 ~30g + 晚餐橄欖油 2 大匙 28g + 魚油/蛋/肉脂肪）', 'daily', '飲食', 7, 1, true),
  ...
  -- === Evening (19:00+) ===
  ('19:00 晚餐 + 低 FODMAP 蔬菜', '蛋白質 40-45g + 菠菜、櫛瓜等低 FODMAP 蔬菜（十字花科留給午餐）。順序：纖維→蛋白→碳水。脂肪：橄欖油 2 大匙（28g）入菜或涼拌。隨餐服用：維他命 C 500mg（半錠）+ 鋅 25mg（半顆，每兩天一次，與 16:00 銅間隔 3hr+）。⚠️ 牛肉日務必減蛋：草飼牛肉 200g ≈ 40-50g 蛋白，不加蛋，確保單餐 ≤50g 避免腎臟短時間代謝壓力。固體食物睡前 3-4hr 結束', 'daily', '飲食', 13, 1, true),
  ('19:30 餐後散步 15 分鐘', '控制餐後血糖最有效的方法，降低胰島素峰值。輕快步行即可，不需高強度。午餐後也建議散步 10-15 分鐘。可結合戶外自然接觸（每週目標 120+ 分鐘）', 'daily', '運動', 14, 1, true),
  ('22:00 睡前補充品', '甘胺酸 3g + 蘇糖酸鎂 + 甘胺酸鎂 100mg + Ashwagandha 600mg（嚴格 8 週用 / 4 週停，在瓶身標記開始日與第 56 天停用日）。⚠️ 第 6 週起密切觀察情緒變化：若出現情緒冷漠（Anhedonia）或早晨無力起床 → 立即進入停用期。停用期替代：甘胺酸鎂 + Cyclic Sighing 維持睡眠品質。22:00 服用確保與 19:00 晚餐蛋白質間隔 3hr+（甘胺酸與蛋白質競爭吸收），同時為腎臟保留排尿緩衝時間', 'daily', '補充品', 15, 1, true),
  ...
  -- === All-day items ===
  ('全天 蛋白質 146g+ 目標', '訓練前乳清 27g + 午餐 40-45g（含每週 1-2 次草飼牛肉 200-300g 替代雞胸）+ 下午豌豆 16g + 晚餐 40-45g ≈ 130-140g。每餐達亮氨酸門檻 2.5-3g。每日 4-5 餐均勻分配，總計 146g+（2.0g/kg）。⚠️ 牛肉日務必減蛋：牛肉 200g ≈ 40-50g 蛋白，晚餐不加蛋，確保單餐 ≤50g。牛肉日額外提供血基質鐵、B12、天然肌酸', 'daily', '飲食', 21, 1, true),
  ...
  ('全天 碳水循環：訓練日 vs 休息日', '重訓日 5-6g/kg (360-430g)、有氧日 3-4g/kg (215-290g)、休息日 2-3g/kg (145-215g，僅 Deload 週或臨時休息)。支持 mTOR 肌肉修復與合成。⚠️ 高碳水日嚴格執行 30-40% 替換為低纖維來源（白米飯、義大利麵、去皮馬鈴薯）以控制總纖維量。優先監控脹氣反應：若出現腹脹→當餐立即減少十字花科蔬菜、增加低纖維碳水佔比。卡路里目標：重訓日 3,100-3,400 kcal', 'daily', '飲食', 24, 1, true),
  ...
  ('全天 鈣攝取確認（目標 1000mg）', '每日確認鈣攝取是否達標 1000mg。主要來源：希臘優格 200-300g（~200-300mg）+ 深綠蔬菜 + 豆腐。⚠️ 若當日飲食鈣攝取確認不足 → 午餐（12:00）隨餐補 1 錠鈣片 500mg（Nature Made Ca+D3+K2）。⚠️ 補鈣當日：鈣片已含 D3 150IU，午餐獨立 D3 應停用（避免疊加超量）。禁止睡前服用（鈣與鎂競爭 DMT1 載體）。午餐鈣 → 16:00 銅 → 19:00 鋅，各間隔 3-4hr+', 'daily', '飲食', 25, 1, true),
...
INSERT INTO plan_items (title, description, frequency, category, sort_order, target_count, is_active) VALUES
  ...
  ('鋅 25mg 每兩天補充', '每兩天隨晚餐服用鋅 25mg（Zinc Picolinate 半顆）。平均每日鋅攝入 12.5mg + 銅 2mg/日 = 鋅銅比約 6.25:1（加上飲食鋅可達 10:1 以上）。與 16:00 銅間隔 3hr+', 'daily', '補充品', 27, 1, true);
...
INSERT INTO products (name, description, usage, price, url, store, category, brand, origin, specs, nutrition, purchase_note, sort_order) VALUES (
  ...
  '線上可訂（常溫配送）。純鈣備用品（鈣從食物攝取為主），250 錠可用非常久。⚠️ 補鈣當日注意：本品每錠含 D3 150IU，午餐獨立 D3 1000IU 應停用，避免長期疊加超量。碳酸鈣+檸檬酸鈣錠劑常溫陰涼處即可穩定保存。適合放辦公桌或隨身攜帶，確保 13:00 補鈣時段可取用。',
  2
);
...
INSERT INTO products (name, description, usage, price, url, store, category, brand, image_url, rating, review_count, sku, specs, nutrition, purchase_note, sort_order) VALUES (
  ...
  'iHerb 直送。⚠️ 8 週用期（56 天）每日 1 顆，每瓶 90 顆可完成 1 個完整週期（56 顆用 + 剩 34 顆留下一輪）。建議在瓶身標記「開始日」與「第 56 天停用日」。停用期 4 週（28 天）替代方案：甘胺酸鎂 + Cyclic Sighing 呼吸法維持睡眠品質。⚠️ 第 6 週起留意情緒冷漠 → 立即停用。若 ALT/AST 異常 → 首位停用本品。',
  36
);
